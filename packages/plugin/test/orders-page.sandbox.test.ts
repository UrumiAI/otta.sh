...
describe("admin Orders console (workerd sandbox)", () => {
	...
	test("INC-13: `Placed` is ABSOLUTE on the list, and the detail states the SAME field in the SAME words", async () => {
		await boot();
		const rows = tableRows(await list());
		// Not `relative_time`, and not the raw wire value either: the operator
		// reads the instant, on the screen they scan for it.
		expect(rows[0]?.createdAt).toBe("10 Jul 2026, 01:00 UTC");
		expect(rows[1]?.createdAt).toBe("11 Jul 2026, 01:00 UTC");
		const placedColumn = columnsOf(findBlock(await list(), "table")).find(
			(c) => c.label === "Placed",
		);
		expect(placedColumn?.format).toBeUndefined();
		// THE POINT OF THE INCREMENT: one click apart, one format. The list row and
		// the detail's identity strip are compared against each OTHER, not each
		// against a literal — a future change to the dialect has to move both or
		// fail here.
		const detail = await open(LIST_ID_1);
		expect(fieldEntries(detail)).toContain(`Placed=${String(rows[0]?.createdAt)}`);
	});

	test("INC-13: the `Placed` column of the customer's OTHER orders agrees with the primary list", async () => {
		await boot();
		const listed = tableRows(await list()).find((r) => r.total === "$20.00");
		const other = groupBlocks(await open("ord-1"), "orders:ord-1:other-orders");
		// Same field, same record, one accordion below the H1 — `SUMMARY_2` is both
		// a row of the primary list and this table's only row.
		expect(tableRows(other)[0]?.createdAt).toBe("11 Jul 2026, 01:00 UTC");
		expect(tableRows(other)[0]?.createdAt).toBe(listed?.createdAt);
	});

	test("INC-13: the sessions table states its two absolute instants in the dialect, labels unsuffixed", async () => {
		await boot();
		const sessions = groupBlocks(await open("ord-1"), "orders:ord-1:sessions");
		// `Signed in` KEEPS `relative_time` — an age, stated on no other surface.
		expect(columnLabels(findBlock(sessions, "table"))).toEqual(["Signed in", "Expires", "Revoked"]);
		expect(tableRows(sessions)[0]?.expiresAt).toBe("8 Aug 2026, 00:00 UTC");
		expect(tableRows(sessions)[0]?.revokedAt).toBe("—"); // unknown stays unknown
	});
	...
	test("the detail is five blocks outside the tabs plus FOUR constant task-named panels at default_tab 0 (D-1..D-4)", async () => {
		...
		expect(labels).toEqual([
			"Status",
			"Total",
			// INC-13: no `(UTC)` suffix — the VALUE states the zone now.
			"Placed",
			"Payment",
			"Order ID",
			"Reconciliation",
		]);
		// INC-13: the console's one timestamp format, never the raw wire value.
		expect(fieldEntries(blocks)).toContain("Placed=10 Jul 2026, 01:00 UTC");
		// The H1 states the SAME instant as a date, and it is this rendering
		// truncated rather than a second one — the detail used to say "placed"
		// twice, in two formats, and an operator had to know they were one field.
		const h1 = String(findBlocks(blocks, "header")[0]?.text ?? "");
		expect(h1).toBe("Order · cust-a · 10 Jul 2026");
		expect("10 Jul 2026, 01:00 UTC".startsWith("10 Jul 2026")).toBe(true);
	...
	test("the Customer group's LABEL carries the answer (D-6), and its body keeps identity, addresses, sessions and other orders — token-free", async () => {
		...
		expect(values).toEqual([
			...
			"Email verified=9 Jul 2026, 00:00 UTC",
		]);
	...
	test("a SHIPPED order with tracking shows the recorded fulfilment read-only, no form", async () => {
		...
		expect(values).toContain("Shipped=11 Jul 2026, 09:00 UTC");
	...
	test("a RESOLVED order shows the recorded disposition read-only, closed, and NO resolve form", async () => {
		...
		expect(values).toContain("Resolved=10 Jul 2026, 02:00 UTC");
	...
	test("an already-cancelled order shows the recorded reason read-only; one cancelled WITHOUT a reason says so honestly — neither offers a control", async () => {
		...
		expect(values).toContain("Cancelled=11 Jul 2026, 09:00 UTC");
		expect(
