...
function deriveCommerce(content: Record<string, unknown>): DerivedCommerce {
	...
	// TITLE: always the CMS content's own top-level `title` column, never a
	// widget field and never whatever a hand-written `commerce.title` says — one
	// source of truth, so the storefront heading and the order line's snapshot
	// cannot drift. Injected here (rather than read inside the parser) so BOTH
	// hooks get it from the single shared derive. `?? ""` collapses an absent /
	// null title into the same non-empty-string rejection an empty one gets:
	// without a title the row is unpurchasable (`PRODUCT_NOT_PRICED` at
	// checkout), so it must fail loudly, never silently sync a NULL title.
	const { body, errors } = parseCommerceFields({ ...bag, title: content["title"] ?? "" });
 ...
 *   - TITLE: taken from the CMS content's own `title`, not the widget bag, and
 *     sent on EVERY upsert. Without it the row's title stays NULL and
 *     `createOrderFromCart` rejects the buyer's checkout with
 *     `PRODUCT_NOT_PRICED`; an empty/whitespace title is a loud validation
 *     rejection, exactly like a float price.
