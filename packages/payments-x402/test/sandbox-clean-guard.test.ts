 ...
 *
 * `@otta-sh/payments-x402` is constructed in-process by the plugin, so its
 * source is loaded inside the workerd sandbox — where a `node:` import is not
 * merely discouraged but unavailable. Its offline HMAC facilitator used to
 * import `node:crypto`.s `createHmac` + `timingSafeEqual`; both are now
 ...
 */
describe("sandbox-clean guard: payments-x402 src carries no node: import (INC-C1)", () => {
