 ...
 *
 * A `FixedClock` makes `updatedAt` identical on every write, which silently turns
 * the port's optimistic compare-and-set into a guard that always passes: a
 * same-document race would then "pass" while proving nothing, because the guard it
 * depends on was never exercised. Real writers read a moving clock, so the race
 * suites do too.
 *
 ...
 * watermarks the guard needs without any bookkeeping in the case itself.
 */
