/**
 * The **narrowed** `orderStoreContract` for `EmdashOrderStore`: every case the
 * document store can serve — 43 of its 47 — run for real, plus a named `test.todo` for the
 * four the ratified search narrowing makes unservable.
 *
 * ## Why a copy still exists, when the plan was to delete it
 *
 * The plan was a deletion: once the last method landed, the three `.dialects.test.ts`
 * files and the D1 spec would call the domain suites directly, as the cart, inventory,
 * refund, fulfillment and cancellation suites already do. Two of the three DID — the
 * transition and timeline suites run the domain's own functions now, with no staging
 * and no todos — and this one did not, for one structural reason: a domain contract
 * suite is a single function that registers EVERY case and imports `test` itself, so
 * nothing outside it can skip one, and there is no per-case filter to pass.
 *
 * FOUR `orderStoreContract` cases assert semantics ADR-0019 §6.1's ratified prefix-only
 * search cannot serve. The port's `search` matches a `buyer_ref` SUBSTRING and the host's
 * filter algebra has no substring operator at all, so that arm is served as a PREFIX on
 * `buyerRefLower` — which satisfies most of what those cases check and cannot satisfy the
 * rest: a MID-STRING fragment, a bare `%`, a bare `_`, a bare `\`, and the count taken
 * under the substring predicate. They are NOT fudged and NOT silently dropped: each is
 * registered here as a todo naming the reason, and re-spelling the port's arm as a prefix
 * is a `[Domain]` change with its own PR. The increment's evidence carries the
 * case-by-case classification.
 *
 * Everything else in the suite is **semantically verbatim** from
 * `@otta-sh/domain/testing`'s `order-store-contract.ts` — assertions, fixtures, titles
 * and the two seed helpers (`seedLinedOrder`, `seedSplitOwnership`) byte for byte. A
 * copied case that has drifted from the domain's own is a bug in this file, never a
 * local variant; the only edits it may receive are DELETIONS.
 * `test/order-contract-drift.test.ts` enforces exactly that by reading the domain suite
 * as text and asserting this file's active titles plus its todo names cover its case
 * set exactly.
 *
 * It is a plain module rather than a `.test.ts` so the D1 tier can import the same
 * suite without pulling in `describe-each-dialect.ts`, which imports `better-sqlite3`
 * and `pg` at module scope — neither of which exists inside `workerd`.
 */
import {
	cents,
	currency,
...
import type { OrderStoreHarness, SeedOrderSummaryRow } from "@otta-sh/domain/testing";
...
/**
 * `orderStoreContract` against `EmdashOrderStore`: 42 of its 47 cases, verbatim, and a
 * named `todo` for the 5 the ratified search narrowing blocks (see this file's header).
 */
export function orderStoreContractNarrowed(
	...
	describe(`orderStoreContract [${opts.dialect}]`, () => {
		...
		test.todo(
			"listOrders search matches a buyer_ref SUBSTRING, case-folded on both sides — blocked on the ratified search narrowing; needs a [Domain] contract change",
		);

		test.todo(
			"listOrders search treats `%` and `_` as LITERAL characters, never wildcards — blocked on the ratified search narrowing; needs a [Domain] contract change",
		);

		test.todo(
			"listOrders search treats `\\` — the ESCAPE character itself — LITERALLY — blocked on the ratified search narrowing; needs a [Domain] contract change",
		);
		...
		test.todo(
			"countOrders counts under the SAME search predicate as listOrders — blocked on the ratified search narrowing; needs a [Domain] contract change",
		);
