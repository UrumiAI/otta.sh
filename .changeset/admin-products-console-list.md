...
- `@urumi/domain`: adds `ProductCommerceStore.listProducts(filter, page)` returning
...
- `@urumi/store-postgres`: implements `listProducts` as a single
...
- `@urumi/service`: adds the internal-token-guarded `GET /admin/products` (filters
...
- `@urumi/plugin`: adds the Products admin page (list with an active/kind/search
  ...
  the plugin defines its own local wire types and never imports `@urumi/domain`
