...
- `@urumi/domain`: widens the `orders.state` machine (adds `processing`/`shipped`/`delivered`/
...
- `@urumi/store-postgres`: migration `0006` (customers, addresses, customer_sessions,
...
- `@urumi/service`: `POST /auth/login/request|verify`, `POST /auth/logout`, `GET /me`,
  ...
  wired.
- `@urumi/plugin`: PUBLIC storefront account routes (`/account/login/*`, `/account/orders`,
