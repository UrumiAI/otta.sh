...
with-CAS/delete). Built entirely on the existing list/detail scaffold and the
admin rules client (both landed in prior slices) — no domain change.
...
`(methodId, currency)` and the rules surface exposes only a
single-currency lookup — the level is a currency-KEYED filter (default
`"USD"`, 0-or-1 rows), not a true multi-row list.
...
rate's own `nonnegative()` (not `positive()`) validation.

Regions are presented honestly: `ShippingZone.regions` is opaque config the
pricing engine never reads (the checkout quote takes an explicit
...
actual referential-guard reason, never a bare failure code. Deleting a rate
