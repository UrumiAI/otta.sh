...
**New package `@otta-sh/admin-presentation`.** Money (`formatMoney`, `formatAmount`, the `Cents`/`Currency` brands, `parseMinorUnitsInput`), the console's single date dialect, the git-style short-id rule, the order-status vocabulary (`cancelled · closed`), the refund confirm/capability copy, the five-outcome list ladder (`listOutcome`, `rowCountLine`) and the Orders screen's authored copy moved out of `@otta-sh/plugin` into a package with no dependencies, no IO, no React and no EmDash — a `depcruise` rule now enforces that it may import nothing but its own relative modules. `@otta-sh/plugin` re-exports every one of them from its original paths, so its existing suites cover the extracted code and neither surface can drift from the other. This is the bill `console-imports-no-workspace-package` recorded as owed: the React tier gets `formatMoney` by SHARING the function, never by writing a second one.

Both surfaces make the SAME list-outcome call, so the count line, the empty-state wording and which state renders at all cannot disagree: a page-2 miss no longer claims the whole collection is empty, and a zero-row page with a cursor behind it renders the scan note beside `Load more` instead of an empty state on top of it.
