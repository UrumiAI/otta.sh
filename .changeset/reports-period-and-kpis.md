...
- The subtitle leads with the active period in absolute dates
  ...
  fall through the dispatcher to a blank console. An unusable range (backwards,
  incomplete, wider than the service's 400-day cap) renders the default period
  with a banner saying why — always a 200.
- All four `stats` slots are used: Revenue, Orders, AOV and Refunded, each
  ...
  and the tile says so rather than showing a figure it cannot know.
- Revenue by day emits the zero-revenue days, so a month of steady sales and a
  month with a three-week hole no longer render identically. The label drops the
  internal "(N buckets)" vocabulary.
