 ...
 *  nothing else — a React page added under id `otta` would hide ALL FIVE of
 *  that plugin's Block Kit pages from the sidebar (the `adminMode` granularity
 *  trap, ADR-0014 Decision 7). All five, not "the others": once the plugin's
 *  adminMode flips to "react", the sidebar keeps only pages that HAVE a React
 *  component, and the five Block Kit ones do not. (ADR-0014's prose counts them
 *  differently — it was written before ADR-0015 retired two; correcting the ADR
 *  belongs to the docs increment, not to this file.) */
export const CONSOLE_PLUGIN_ID = "otta-console";

/** The Block Kit plugin that keeps its five screens. */
 ...
 *  assertion it backs — that a React page has NOT hidden the five Block Kit
